...
class ParticipationViewSet(viewsets.ModelViewSet):
    ...
    def student_participation(self, request):
        ...
        participation_records = Participation.objects.filter(student_id=student_id, period=period)
        serializer = self.get_serializer(participation_records, many=True)
        return Response(serializer.data)
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"detail": f"Error updating participation record: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST
            )
